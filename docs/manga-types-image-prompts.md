...
> **2026-08-31 更新（PR #26）: C-1〜C-3 のプレースホルダーはすべて実写画像へ差し替え済み。**
> 予告編3枚＝E-01〜E-03、形式プレビュー4枚＝B節 #08〜#11、CTAポスター＝E-04。
> `manga-types.html` には `<picture>` が直接入っており、以下の「差し替え手順」「HTMLコメント」
> 「CSSのみ」「未使用のまま残置」は**差し替え前の記録**。作業時はこの節ではなく D/E 節と
> `SPEC.md` §1（マンガの種類の行）を参照する。画像を作り直すときは**同名で上書き**するだけでよい
> （PNG は 768px 幅に縮小し、生成時の C2PA 来歴メタデータを除去して保存する）。
...
3. （済）`manga-types.html` の `<picture class="mt-trailer-frame__glyph">` が直接参照しているので、
   同名で上書き保存するだけで反映される（HTML側の作業は不要）
...
ポスターらしいレンダリングにします。**E-01〜E-03とは別の、新しいチャットで**行ってください
...
3. （済 2026-08-31）形式プレビュー4枚（1コマ/4コマ/ストーリー型/縦読み）は **B節の #08〜#11**
   （汎用人物・マスコットなし）で生成・配置済み
4. （済 2026-08-31）コード側の反映も完了（形式プレビュー・CTAポスターとも `<picture>` で直接参照）。
   PNG フォールバックは 768px 幅に縮小し、C2PA 来歴メタデータを除去してから保存する（2026-09-03）
