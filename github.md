# github.md — BizManga リポジトリ Git 運用ルール

> **このファイルが BizManga の Git ルールの正本です。git 操作をするときは毎回ここを参照します。**
> 初めてこのリポジトリで作業する人は、最初に必ず一読してください。
> ContentsX リポジトリにも同じ内容の `github.md` があります（差分はリポジトリ名・デプロイ先・ビルドBotだけ）。
