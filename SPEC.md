...
| マンガの種類 | `manga-types.html` | `bm-i18n` + `bm-nav` + `bm-kinsoku` + `bm-lp-v2.js` + **`bm-manga-types.js`** | **2026-08-31 「企業の物語を上映する映画館」版へ全面リデザイン**（2026-08-24版のヒーロー内キャラ+吹き出し+`.mt-hero-index`帯を廃止し予告編コンセプトへ差し替え。CHAPTERS 01–07本体・相談セクション・FAQ本文の趣旨・RELATED 8LP・全リンク/電話/LINEは維持）。<br>構成: **①PROLOGUE**（`.mt-hero`。英字ラベル `PROLOGUE / 00`、見出し「御社の『伝わらない』を、物語の予告編に。」、右側に**3コマ予告編** `.mt-hero-trailer`〈フィルムのパーフォレーション付きセル `.mt-trailer-frame`×3、中身はインラインSVGピクトグラム（吹き出し+×→電球→吹き出し+チェック）。実写素材は未確定のためCSS/SVGのみで組み、差し替え案は [docs/manga-types-image-prompts.md](docs/manga-types-image-prompts.md) C-1節に整理〉、CTA「プロローグから物語を選ぶ」は `#chapters` へ（`html{scroll-behavior:smooth}` 既存のため追加JS不要）。キャラ`character.webp`+吹き出しはHeroから撤去（相談セクションの`character-cta.webp`のみ残す）→**①b BRIDGE**（`.mt-bridge`。大きな帯ではなく1行の接続表現「PROLOGUE END — 7つの物語から、本編を選ぶ」のみ。生成り/白ベースで黒背景不使用）→**②CHAPTERS 01–07**（`#chapters`。**現行デザイン・章名7つ・操作方法を完全維持**）→**③相談セクション**（`.mt-consult`。**変更なし**）→**④SCREENING FORMAT**（`#formats`。旧・罫線区切りの4列テキストリストを**上映プログラムのチケットUI**に刷新: 左に4枚のチケット状ボタン `button.mt-ticket`（半円の「もぎり跡」を`::before/::after`で表現、選択中は`--mt-accent-deep`の枠+チェック）、右に選択中の詳細 `article.mt-ticket-panel`（コマ割りプレビューは**画像不使用・CSSのみ**で描画 `.mt-ticket-preview--{teaser,short,feature,mobile}` — 1コマ=単一ボックス／4コマ=縦4分割／ストーリー型=ずらし重ねのページ風／縦読み=スマホ枠+縦積みコマ、+推奨ページ数・主な掲載場所の`dl`、+おすすめの組み合わせ1件〈マンガ広告×1コマ／How toマンガ×4コマ／採用マンガ×ストーリー型／商品紹介マンガ×縦読み〉）。デフォルト選択は**03 ストーリー型**。章セレクタと同じ設計（`data-mt-formats`コンテナ、JS未動作時は4形式とも縦に並ぶ、`bm-manga-types.js`の`initFormats()`が`aria-pressed`+`aria-live`+↑↓←→キーを担当）。**実装上の注意**: `.mt-ticket-list`はflexで、実際のflexアイテムは`<li>`（中の`button.mt-ticket`ではない）ため、レスポンシブの列数指定（900px以下2列/560px以下1列）は`.mt-ticket-list > li`側に付けること（`.mt-ticket`に付けると意図せず3列などに崩れる。実装時に一度踏んだ）→**⑤AFTER TALK / Q&A**（`#faq`。`lpv2-faq`を流用、英字ラベルは共通`.lpv2-eyebrow`を`lpv2-section-head`内に追加。質問4つを刷新〈4コマとストーリー型は/採用マンガと会社紹介マンガは/縦読み作品を多言語展開できますか/ジャンルもページ数もまだ決まっていません〉、**FAQPage JSON-LDも同一文言に同期済み**。比較ガイドasideは維持）→**⑥NOW SHOWING**（`lpv2-related`流用、英字ラベル`NOW SHOWING / 01–08`追加、カードに上辺4pxのオレンジバー`::before`を足してポスター調に。列数(4/2/1)・リンク8本は既存のまま）→**⑦NEXT PRODUCTION MEETING**（`.mt-cta`。2カラム化: 左に見出し「次に予告編になるのは、御社の物語。」+CTA2本（`/contact`・LINE、リンク先は既存を再利用）、右に**COMING NEXTポスター** `.mt-cta-poster`（スポットライト状のradial-gradient + シルエット人物はCSSの円+角丸台形のみ、文字「YOUR STORY / COMING NEXT / Produced with BIZMANGA」は全てHTML、画像への文字焼き込みなし）。<br>**a11y/性能**: 新規要素も`aria-pressed`・`aria-live="polite"`・フォーカスリング・`prefers-reduced-motion`対応を章セレクタと同基準で踏襲。新規CSS/SVGのみで画像リクエスト増加なし＝CLSリスクなし。`.lpv2-eyebrow`はFAQ/RELATEDで初使用のため`.mt-v2`スコープで`--mt-accent-deep`に上書き（既存のAA方針を踏襲）。Playwrightで3ブレークポイント(1440/820/375)の横スクロール無し・章/形式切替・FAQ開閉・キーボード操作（矢印キー、Enter）を確認済み、コンソールエラーなし（広告/解析タグの`requestfailed`はサンドボックス起因で無関係）。<br>**未参照のまま残置**: `images/manga-types/format-{1koma,4koma,story}.png`（+webp）は今回も不使用（コマ割りプレビューはCSSのみ）。7ジャンルの挿絵`type-*.png`は引き続き使用。 | **2026-08-31 追加調整（ダーク背景化は行わず、既存の白/生成り/オレンジ基調のまま微調整）**: ①相談セクションのラベル/見出しを「INTERMISSION / STORY CONSULTATION」「次のチャプターに迷ったら、課題だけ聞かせてください。」に変更（レイアウト・数値・CTAは維持）。②予告編3コマを実写差し替え前提の構造に変更（`.mt-trailer-frame__art`固定アスペクト3:2＋`.mt-trailer-frame__glyph`がimg差し替えでobject-fit:coverになる設計。推奨ファイル名`trailer-{01-problem,02-idea,03-result}.png/webp`はdocs/manga-types-image-prompts.md C-1節）。③SCREENING FORMATのプレビューに効果音「パッ！」・短いキャッチコピー・起承転結の吹き出し・変化を示す2点等を追加し「マンガの仮ネーム」寄りに強化、形式切替時に150〜180msのopacity+translateフェード（prefers-reduced-motionで無効化）。④NOW SHOWINGのカードをaspect-ratio 4/5の縦長ポスターに変更し番号表記を「FILM 0X」に、右上に控えめな網点円モチーフを追加。⑤旧`lpv2-sr-only`の非表示SEO文言を可視の`.mt-hero-kicker`に変更し、退役したH1文言「企業の数だけ、伝えるべき物語がある。」をCHAPTERSセクションのリード文に自然に配置。meta title/description/canonical/構造化データは無変更。フローティングCTAとの重なりはPlaywrightで4幅（1440/820/375/320）検証し問題なし。
