...
| マンガの種類 | `manga-types.html` | `bm-i18n` + `bm-nav` + `bm-kinsoku` + `bm-lp-v2.js` | **2026-08-19 LP v2 デザインへ全面刷新**（旧・全画面キャラ+ラベル散りばめの独自ヒーロー `mt-hero` 等を廃止）。ヒーローは product-manga / strength と完全同型（`recruit-hero-v2` 全面背景 + 左コピー + pill CTA 2本）。構成: Hero→CH01 目的で選ぶ早見表（`.lpv2-related-card` を流用した4問クイズ「応募を増やしたい」「CVRを上げたい」「新人教育を効率化」「信頼・想起を高めたい」→各ジャンルへアンカージャンプ）→CH02 7ジャンルグリッド（`.lpv2-merit-card` を3列化する `.mt-genre-grid`、7枚目=奇数枚は中央寄せ。各カードはクリックで詳細トグル、関連する用途別LP・コラムへのリンクを含む）→CH03 ページ構成比較（1コマ/4コマ/ストーリー型の3カード、各カードに挿絵 `.mt-length-card__illust`、CSS側で4:3中央トリミング）→BRIDGE(縦読み・多言語対応の一文＋インバウンド漫画への導線)→CH04 FAQ(4問、既存FAQPage JSON-LDと同一文言)+比較ガイドaside→RELATED(用途別LP 8本、product-manga起点)→END CTA。CSSは `css/bm-lp-v2.css` + 薄いアドオン `css/manga-types.css`（全セレクタを `body.mt-v2` でスコープ）。**7ジャンルの挿絵 `images/manga-types/type-{founding,recruit,casestudy,howto,product,training,profile}.png`（+webp）は2026-08-21にChatGPT Image 2.0生成の本番イラストへ差し替え済み**（水彩+ink画風、recruit-manga/product-manga画像を参照基準に統一。webpはPillowで再生成、品質82）。**ページ構成カット `images/manga-types/format-{1koma,4koma,story}.png`（+webp）は当初CSSのコマ割り模式図（`.mt-length-mock`、廃止済み）だったが、同日ユーザー要望で実イラストに変更・生成完了**（1コマ=スマホの1枚パネルに驚くシーン／4コマ=2×2の紙面を指でなぞるシーン／ストーリー型=分厚い冊子を読みふけるシーン。webpはPillowで再生成、品質82）。生成プロンプトと画風の正本は [docs/manga-types-image-prompts.md](docs/manga-types-image-prompts.md)（A=7ジャンル7枚、B=ページ構成カット3枚。strength-image-prompts.mdと同一の水彩+ink画風を指定）。 |
