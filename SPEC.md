...
| 料金 | `pricing.html` | —（共通JSのみ） | **2026-08-21 Hero新デザイン追加**（`.pr-*`クラス）: 旧`.bm-img-hero`型ページヘッダーを廃止し、左=コピー+ビズちゃん（指差しポーズ+吹き出し「2つのプランから選べるよ！」）／右=料金カード2枚（フル漫画家/ハイブリッド）／下部=特徴バッジ4項目、という2カラムHeroに刷新。背景は`material/images/pricing/pricing-hero-bg-manga.webp`。Hero直後の詳細版セクションとは意図的に情報が重複（Hero=サマリー、詳細セクション=本数割引表・機能一覧を含むフル情報という二段構成）。ヘッダーは他ページ共通の通常形状を維持。**2026-08-19 全面刷新**: 2プランカード型料金表（フル漫画家プラン ¥33,660/ページ〜・原稿料別途33,660円/10P目安約37.0万円／ハイブリッドプラン ¥25,740/ページ〜・原稿料別途25,740円/10P目安約28.3万円、いずれも税抜）+ 3バッジ + プラン別本数割引ミニ表2枚（7P基準。フル漫画家: 1本33,660/3本30,320/5本28,260円/P、ハイブリッド: 1本25,740/3本23,200/5本21,600円/P。原稿料は各プラン単価と同額を別途）+ 特徴ストリップ4項目 + 比較ガイドaside。カードCTAは `/contact?plan=full\|hybrid`（§2.2のプリフィル連携）。旧3プラン体系（ライト/スタンダード/プレミアム、2026-07-02版）は廃止。「業界最安値級」表現は削除し「大手の約1/5の価格」に統一（値上げに伴う訴求見直し）。Service JSON-LDのOfferは表示価格と同期（lowPrice 25740 / highPrice 33660） |
...
- **2026-08-21 追加調整**（ユーザーフィードバック反映）: ①ビズちゃんを340px→380pxへ拡大（PC）、モバイルも260px→340pxへ拡大 ②吹き出し形状を「しっぽ付き角丸長方形」から完全な正円（`aspect-ratio:1/1; border-radius:50%`）に変更、位置も本文と重ならないよう`top`を4%→22%に下げて調整 ③料金カード2枚を約20%縮小（`.pr-plan-card`のpadding/font-sizeを一律縮小、`.pr-plan-cards`に`max-width:640px`を追加しHero右側に寄せて配置）
- **2026-08-22 目標デザイン画像へのピクセル一致対応** ⭐: 目標hero.png（1536×1024）から各要素の座標を画像解析で実測し、PC 1536px幅でその座標を再現するようレイアウトを全面再設計。
  - **測定した目標座標**（1536×1024基準）: フル漫画家カード x610-1009・y251-788（399×537）／ハイブリットカード x1031-1428／カード間gap 22px／タブ 180×42px（カード上端より30px上）／バッジ帯 x433-1428・y852-982（995×130、pill形状）／ビズちゃん頭頂 y520／吹き出し 直径145px・中心(508,712)
  - **DOM構造変更**: `.pr-hero-badges-wrap`（Hero直下の別ブロック）を廃止し、`.pr-hero-right`（右カラム）内に`.pr-plan-cards`と`.pr-hero-badges`を格納。バッジ帯は`margin-left:-177px`で左カラム側へ張り出させ、目標どおり「ビズちゃんの右から始まる」配置を実現
  - **Hero高さ**: `min-height: min(927px, calc(100vw * 0.604))` で目標画像の縦横比を維持（画面が横長でもコンテンツ比率を崩さず、余った左右は背景がcoverで埋める）
  - グリッド比 39.5%/60.5%、`.pr-hero-inner` padding `0 48px 0 20px` でコピー左端を目標の x=68 に一致
...
- **2026-08-22 可読性・モバイル最適化**（ユーザーフィードバック）: ①説明文が背景・キャラと同化して読みにくいため、`-webkit-text-stroke` + `paint-order: stroke fill` + `text-shadow` で白フチを付与 ②全体をさらに10px上寄せ ③**モバイル(900px以下)のレイアウトを刷新**: ビズちゃんを240px幅（640px以下は210px）に縮小してコピーとの重なりを解消／料金カード2枚を縦積みから**横並び2カラム**に変更（カード内フォント・余白も一括縮小し、360px幅でも横スクロールなしを実測確認）／**4項目の特徴バッジ帯はモバイルでは非表示**（PCのみ表示）。なお画面下端の固定CTAバー（`.bm-fab`）がカード価格に重なる件は、全ページ共通の既存挙動としてユーザー判断で現状維持
