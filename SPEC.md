...
## 1b. ビズアニメ（BIZ ANIME / ボイスコミック紹介）⭐進行中

漫画にナレーション・BGM・効果音・動きを加えた動画版＝ボイスコミックのサービス紹介ページ。
ビズマンガのブランド × 映像制作スタジオのシネマティック感（BLACK / CHARCOAL / ORANGE / WHITE）で、
「漫画 → 声 → 音 → 動画」が一目で伝わることを狙う。他ページと違い**このページだけ黒基調**。

**現状: Hero セクションのみ実装（2026-08-21）。PC先行でスマホは未着手。**
...
単純な3カラムにせず absolute で意図的に重ねる。右下に VOICE / MUSIC / SFX / MOTION。

### 実装上の必須ルール（触る前に読む）
- **テキストは画像化しない**。BIZ ANIME・キャッチ・CTA・VOICE/MUSIC/SFX/MOTION はすべてHTML。画像素材はキャラ・プレイヤー・背景・波形装飾のみ（SEO/a11y/レスポンシブのため）
...
- **ヘッダーにサービス名を足さない**。ナビ7項目＋言語切替＋CTA＋アイコン2個で、1920pxでもロゴ右の余白は約70pxしかない。要素を差し込むとナビが溢れて言語切替の下に潜る（#003/#014と同種の事故）。`.ba-header-brand` は実装済みだが `display:none` で封印してある
- **ロゴは `filter: brightness(0) invert(1)` で白へ反転**。既存ロゴは濃色前提のPNGで、黒背景では沈む
- **Hero上では追従CTA（`.bm-fab`）を隠す**。左下のCTAと重なるため
- キャラの脚は画像幅の93%まで及ぶので、バッジ列を余白で逃がすことはできない。`z-index` と `text-shadow` で可読性を確保する

### 未実装（次にやること）
- スマホ／タブレット対応（`responsive-rules` スキルで実装。現状 1024px以下は縦積みの暫定フォールバック）
- Hero より下のセクション全体
- ボイスコミック一覧（WP `/voice-comics` 連携）。**仕様は決定済み**: WPに専用投稿タイプを新設し、YouTube URL を貼るだけで一覧に増える運用。カード内でそのまま再生（YouTubeへ遷移させない）。プラグインは別リポジトリ(PRIVATE)＋お名前.com手動アップロードが必要（[BUGS #002](../BUGS.md)）
- ナビへの導線追加（`js/bm-nav.js` の `NAV_ITEMS`）。**ヘッダーは事故多発箇所なので、追加したらスマホ幅でハンバーガー開閉を必ず実測する**
