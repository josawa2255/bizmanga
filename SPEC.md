...
（**VOICE/MUSIC/SFX/MOTION の4機能UIはスマホでは非表示**。2026-08-22 指示。内容はHeroより下のセクションで扱う）
...
- **追従CTA(`.bm-fab`)もスマホでは非表示**。Heroのスクロール量が200px程度しかなく、画面下に固定表示するとどの位置で止めても VOICE/MUSIC/SFX/MOTION に重なる（余白調整では回避不能と実測で確認）。連絡手段はハンバーガー内のLINE/電話で確保。`/contact` `/biz-library` にも同じ前例あり
...
- **Heroの下padding は 110〜150px**。共通の追従CTA(`.bm-fab` 高さ56px)が画面下に固定されるため、これを削ると最下部で VOICE/MUSIC/SFX/MOTION が隠れる
