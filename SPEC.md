...
| マンガの種類 | `manga-types.html` | `bm-i18n` + `bm-nav` + `bm-kinsoku` + `bm-lp-v2.js` + **`bm-manga-types.js`** | **2026-08-31 「企業の物語を上映する映画館」版へ全面リデザイン（PR #26、2026-09-03 レビュー修正込み）**。2026-08-24版のヒーロー（公式キャラ+吹き出し+`.mt-hero-index`索引帯）を廃止し、予告編→本編→上映形式→上映後トーク→上映作品→次回作の制作会議、という上映体験に統一。CSSは共通 `css/bm-lp-v2.css` のトークンの上に `css/manga-types.css`（`body.mt-v2` スコープ、`mt-*` 名前空間）。基調は白 / 生成り / オレンジだが、**最終CTAだけはダーク（`--mt-ink`）の全幅セクション**（黒背景不使用の原則の例外）。<br>構成: **①PROLOGUE**（`.mt-hero`。ラベル PROLOGUE/00、H1「御社の／「伝わらない」を、／物語の予告編に。」（`<br>`2つで3行。「御社の「伝わらない」を、」12文字は 660px 列にも 375px にも入らず「を、」が孤立行になるため意味の区切りで分けた）、SEOキッカー `.mt-hero-kicker`（旧 `lpv2-sr-only` を可視化）、CTA2本。右に**3コマ予告編** `.mt-hero-trailer`＝`.mt-trailer-frame`×3（フィルムのパーフォレーション付き 3:2 枠 `.mt-trailer-frame__art` の中に `<picture class="mt-trailer-frame__glyph">` で実写カット `images/manga-types/trailer-{01-problem,02-idea,03-result}.webp/png`、`object-fit:cover`。CUT 01 のみ `loading="eager" fetchpriority="high"`＝PCでファーストビュー内）。グリッドは `align-items:start`（ストリップが約900pxと高く、center だとコピーとCTAが押し下げられて 1366×768 でCTAが画面外に落ちる）→**①b BRIDGE**（`.mt-bridge`。「PROLOGUE END — 7つの物語から、本編を選ぶ」の1行のみ）→**②CHAPTERS 01–07**（`#chapters`。**2026-08-24版を完全維持**: 左 `button.mt-chapter-row`×7／右 `article.mt-chapter-panel`（挿絵 `type-*.png`）、`aria-pressed`+`aria-live="polite"`、↑↓←→キー、`#mt-{founding,recruit,casestudy,howto,product,training,profile}` の直リンクで章を開いて `scrollIntoView` で位置補正。JS無効時は7章すべて縦並び。リード文に旧H1「企業の数だけ、伝えるべき物語がある。」を配置）→**③相談セクション**（`.mt-consult`。ラベル「INTERMISSION / STORY CONSULTATION」、見出し「次のチャプターに迷ったら、課題だけ聞かせてください。」に変更した以外は 2026-08-24版のまま: 招待状パネル `.mt-consult-panel`、数値3件 `.mt-consult-facts`、導線3つ（`/contact` + LINE + `tel:`）、`character-cta.webp` + 吹き出し `.mt-bubble.mt-bubble--right`（**基本ルール `.mt-bubble` はこのセクション専用として CSS の相談ブロック内に置く**。PR #26 当初はヒーロー撤去と一緒に消えて吹き出しが無装飾になっていた）。SP(≤420px)ではリード文の `<br>` を非表示）→**④SCREENING FORMAT**（`#formats` `.mt-tickets[data-mt-formats]`。左に4枚のチケット状ボタン `button.mt-ticket`（もぎり跡 `::before/::after`、選択中は `--mt-accent-deep` 枠+✓）、右に `article.mt-ticket-panel`（**実写プレビュー4枚** `format-{1koma,4koma,story,vertical}.webp/png` を `.mt-ticket-preview`（3:2、`object-fit:cover`）に表示 + 推奨ページ数／主な掲載場所の `dl` + おすすめの組み合わせ1件）。デフォルトは **03 ストーリー型**。`initFormats()` が章セレクタと同じ設計（`aria-pressed`/`aria-live`/矢印キー、JS無効時は4形式とも縦並び、切替時 0.18s フェード・`prefers-reduced-motion` で無効）。ハッシュ深リンクは章のみ（`#mt-format-*` は対象外＝スクロールもしない）。900px以下は2列（横gap 24px）、560px以下は1列）→**⑤AFTER TALK / Q&A**（`#faq`。`lpv2-faq` 流用 + `.lpv2-eyebrow`。質問4つ〈4コマとストーリー型／採用マンガと会社紹介マンガ／縦読み作品を多言語展開／ジャンルもページ数も未定〉、**FAQPage JSON-LD は同一文言に同期済み**。比較ガイド aside 維持）→**⑥NOW SHOWING**（`lpv2-related` 流用。番号「FILM 0X」、上辺4pxオレンジバー `::before` + 右上に網点円 `::after`。**縦長固定はしない**（他8LPと同じ内容に沿った高さ）。ホバーで共通カードが黒地になるため番号は `--lpv2-accent` に戻してAA維持。列数(4/2/1)・リンク8本は既存のまま）→**⑦NEXT PRODUCTION MEETING**（`.mt-cta` ダーク背景。左=「NEXT PRODUCTION MEETING / 30 MIN」+ 見出し「次に予告編になるのは、御社の物語。」+ CTA2本（`/contact`・LINE）、右=`.mt-cta-poster`〈`cta-poster-character.webp/png`（背景透過のマスコット、`object-fit:contain`）+ スポットライト + HTML文字 YOUR STORY / COMING NEXT / Produced with BIZMANGA〉。720px以下は1カラム中央揃え）。<br>**画像**: 新規8枚（trailer×3 / format×4 / cta-poster-character）はすべて `<picture>`（webp を実配信、png はフォールバック。**png は 768px 幅に縮小し、生成時の C2PA 来歴メタデータ（caBX チャンク）を除去して保存する**＝2MB×8 のリポジトリ肥大と「AI生成」表示の回避）。7ジャンルの `type-*.png` は継続使用。生成プロンプトの正本は [docs/manga-types-image-prompts.md](docs/manga-types-image-prompts.md)（C節は差し替え前の記録）。<br>**実装上の注意（PR #26 レビュー 2026-09-03 で実際に踏んだ4点）**: (a) 行方向 flex の子に `aspect-ratio` を付けても親の `align-items:stretch` が優先され縦に伸びて `cover` で左右が切れる → `.mt-ticket-preview` に `align-self:flex-start`。(b) `.mt-ticket-list` の flex アイテムは `<li>`（`button.mt-ticket` ではない）なので列数指定は `li` 側に付ける。もぎり跡は枠外に7pxはみ出すので2列時の横gapは24px以上。(c) セクションを撤去するとき、`.mt-bubble` のように**他セクションが共用する基本ルールを一緒に消さない**。(d) `#mt-*` ハッシュのスクロール補正は `select()` が章に一致したときだけ行う（`#mt-hero-title` 等で CHAPTERS に飛ばさない）。<br>**a11y／コントラスト**: 2026-08-24版の方針を継承（白抜き文字を敷く面と 18.66px 未満のオレンジ文字は `--mt-accent-deep`、大きい文字は #e85500、`prefers-reduced-motion` で transition/animation 無効、フォーカスリング `--mt-focus`）。予告編ラベル「TRAILER — 3 CUTS」は黄色だと白地で 1.4:1 のため `--mt-accent-deep`。**i18n**: セクション見出し・ヒーロー・相談・CTA・予告編キャプションは `data-ja/data-en`。チケット本文・形式詳細・FAQ本文・NOW SHOWING カード本文は未翻訳（2026-08-24版と同じ範囲）。<br>**検証（2026-09-03）**: Playwright(Chromium) 320/375/390/412/448/640/768/820/1024/1100/1366/1440/1920px で横スクロール無し・コンソールエラー無し・ローカル404無し。ヒーローCTAは 1366×768 / 1280×800 / 1440×900 でファーストビュー内。章/形式の切替（クリック・矢印キー）、`#mt-recruit` 深リンク、FAQ開閉、EN切替の動作確認済み。 |
