...
| **用途別LP 8本** | `product-manga.html` / `recruit-manga.html` / `manga-ad-lp.html` / `company-manga.html` / `sales-manga.html` / `training-manga.html` / `inbound-manga.html` / `ir-manga.html` | `bm-i18n` + `bm-nav` + WP API | 2026-04-26 公開。SEO中核。共通テンプレ `css/bm-lp-template.css`。各LPは Hero→Formats→Problem→Bridge→Merit→Case Study(WP API動的)→ビズ書庫埋込→**制作フロー(6 Step, HowTo Schema)**→**FAQ(8問, FAQPage Schema)**→**関連LP相互リンク7本**→Ending の構成。JSON-LD 5種: WebPage + Service + BreadcrumbList(2階層) + HowTo + FAQPage、`@id` で相互参照、`inLanguage: ja-JP`、`datePublished/dateModified: 2026-04-27`、Service に `url` + `audience` + `offers(/contact)`。FAQ は共通6問+LP固有2問の計8問構成。1位獲得が目標で `tools/rank-tracker.py` でKW追跡中。**2026-05-09 商品紹介LP可視性向上リファクタ**: ヒーローに `pm-hero-sub`(サブコピー) + `pm-hero-badges`(¥15,800〜/125項目/最短2週間 のトラスト3指標) + `pm-hero-ctas`(主CTA + 副リンク) を追加し高さを 560→680px へ。MERITセクションに `pm-merit-section` クラスで紺ダークバンド化(背景 #14213d + 二重ラジアルグラデ装飾) し `pm-merit-num` を白丸→オレンジ丸バッジ(56×56)に。MERIT直後に `pm-stats-section` (EFFECT IN NUMBERS) を新設、CVR/滞在/読了の3数値タイル(`pm-stat-tile`、左に大数字+右にラベル/補足)。PAINカードに左ボーダー4px +シャドウ追加。FLOW(`pm-flow-list-v`)に左縦コネクタ線(linear-gradient orange→fade)+ 円形バッジ(48px 白底オレンジ枠)。エンディングに `pm-end--rich` モディファイア + `pm-end-points`(30分無料相談/発注義務なし/NDA対応の3つの安心ポイント・チェックマーク付カード)。商品紹介LP のみ HTML 変更、CSS は共通テンプレに追加(他7LPは現状維持、必要に応じて順次同様の HTML 拡張可) |
