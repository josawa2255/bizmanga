...
| マンガの種類 | manga-types.html | bm-manga-types.js（**2026-08-31 「企業の物語を上映する映画館」版へ全面リデザイン**〈PR #26、2026-09-03 レビュー修正込み〉: PROLOGUE〈3コマ予告編・実写〉→BRIDGE→CHAPTERS 01–07〈2026-08-24版を完全維持〉→相談〈INTERMISSION〉→SCREENING FORMAT〈チケットUI・実写プレビュー4枚〉→AFTER TALK/Q&A〈FAQ4問・JSON-LD同期〉→NOW SHOWING〈8LP、縦長固定なし〉→NEXT PRODUCTION MEETING〈**ダーク背景**CTA+マスコットのポスター〉。CSSは `css/manga-types.css`（`body.mt-v2` スコープ）、新規画像8枚は `images/manga-types/`（webp配信・pngは768px縮小フォールバック、C2PAメタデータ除去済み）。**各セクションの仕様・実装上の注意（flex stretch と aspect-ratio／`.mt-ticket-list > li`／`.mt-bubble` 共用ルール／`#mt-*` ハッシュ）の正本は SPEC.md §1 の同行**。画像プロンプトは docs/manga-types-image-prompts.md（C節は差し替え前の記録）） |
