...
- 必ず `python3 tools/smoke-manga-wp.py --base http://127.0.0.1:8127`（ローカル配信）を全 PASS にしてから、結果を添えて平澤さんのOKをもらい `--approve human` でマージ。**この領域は `--approve ai-clean` 不可**（ゲート側でも拒否）
